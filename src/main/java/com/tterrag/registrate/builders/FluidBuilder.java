...
/**
 * A builder for fluids, allows for customization of the {@link ForgeFlowingFluid.Properties} and {@link
 * FluidAttributes}, and creation of the source variant, fluid block, and bucket item, as well as data associated with
 * fluids (tags, etc.).
 *
...
public class FluidBuilder<T extends ForgeFlowingFluid, P> extends AbstractBuilder<Fluid, T, P, FluidBuilder<T, P>> {
    ...
    private final List<Named<Fluid>> tags = new ArrayList<>();
    ...
    /**
     * Create a new {@link FluidBuilder} and configure data. The created builder will use the default attributes class
     * ({@link FluidAttributes}) and fluid class ({@link ForgeFlowingFluid.Flowing}).
     *
     * @param <P>            Parent object type
    ...
    /**
     * Create a new {@link FluidBuilder} and configure data. The created builder will use the default fluid class
     * ({@link ForgeFlowingFluid.Flowing}).
     *
     * @param <P>               Parent object type
     * @param owner             The owning {@link AbstractRegistrate} object
     * @param parent            The parent object
    ...
    /**
     * Create a new {@link FluidBuilder} and configure data. Used in lieu of adding side-effects to constructor, so that
    ...
    /**
     * Modify the attributes of the fluid. Modifications are done lazily, but the passed function is composed with the
     * current one, and as such this method can be called multiple times to perform different operations.
     *
    ...
    /**
     * Modify the properties of the fluid. Modifications are done lazily, but the passed function is composed with the
     * current one, and as such this method can be called multiple times to perform different operations.
     *
    ...
    /**
     * Create a standard {@link ForgeFlowingFluid.Source} for this fluid which will be built and registered along with
     * this fluid.
     *
     * @return this {@link FluidBuilder}
    ...
    /**
     * Create a {@link ForgeFlowingFluid} for this fluid, which is created by the given factory, and which will be built
     * and registered along with this fluid.
     *
    ...
    /**
     * Create a standard {@link LiquidBlock} for this fluid, building it immediately, and not allowing for further
     * configuration.
     *
     * @return this {@link FluidBuilder}
    ...
    /**
     * Create a standard {@link LiquidBlock} for this fluid, and return the builder for it so that further customization
     * can be done.
     *
    ...
    /**
     * Create a {@link LiquidBlock} for this fluid, which is created by the given factory, and return the builder for it
    ...
    /**
     * Create a standard {@link BucketItem} for this fluid, building it immediately, and not allowing for further
     * configuration.
     *
     * @return this {@link FluidBuilder}
    ...
    /**
     * Create a standard {@link BucketItem} for this fluid, and return the builder for it so that further customization
     * can be done.
     *
    ...
    /**
     * Create a {@link BucketItem} for this fluid, which is created by the given factory, and return the builder for it
     * so that further customization can be done.
     *
     * @param <I>     The type of the bucket item
     * @param factory A factory for the bucket item, which accepts the fluid object supplier and properties and returns
     *                a new item
     * @return the {@link ItemBuilder} for the {@link BucketItem}
     */
