...
public class RegistrateRecipeProvider extends RecipeProvider implements RegistrateProvider, Consumer<FinishedRecipe> {
    ...
    public static InventoryChangeTrigger.TriggerInstance hasItem(Tag<Item> tag) {
        return RecipeProvider.has(tag);
    }

    public static InventoryChangeTrigger.TriggerInstance hasItem(ItemPredicate... predicate) {
     ...
     * @param source
     * @param output
     * @deprecated Broken, use {@link #storage(NonNullSupplier, NonNullSupplier)} or {@link #storage(DataIngredient,
     * NonNullSupplier, DataIngredient, NonNullSupplier)}.
     */
    @Deprecated
    ...
    @Override
    public void saveAdvancement(HashCache cache, JsonObject cache2, Path advancementJson) {
        super.saveAdvancement(cache, cache2, advancementJson);
    }
