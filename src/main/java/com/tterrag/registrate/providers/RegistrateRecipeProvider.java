...
public class RegistrateRecipeProvider extends RecipeProvider implements RegistrateProvider, Consumer<IFinishedRecipe> {
    ...
    public InventoryChangeTrigger.Instance hasItem(MinMaxBounds.IntBound amount, Tag<Item> tagIn) {
        return this.hasItem(ItemPredicate.Builder.create().tag(tagIn).count(amount).build());
    }
